#!/bin/bash
# Script d'installation / désinstallation de la règle udev backlight

RULE_FILE="/etc/udev/rules.d/90-backlight.rules"

echo "Gestion de la règle udev backlight..."

# Vérifie si la règle existe déjà
if [ -f "$RULE_FILE" ]; then
    echo "Une règle udev est déjà présente."

    # Demande à l'utilisateur s'il veut la supprimer
    read -p "Voulez-vous la supprimer et restaurer les permissions normales ? (o/n) : " choice

    if [ "$choice" = "o" ] || [ "$choice" = "O" ]; then
        echo "Suppression de la règle udev..."

        # Suppression du fichier de règle
        sudo rm -f "$RULE_FILE"
        ...
        echo "✔ Règle supprimée et système restauré."
    else
        echo "Aucune modification effectuée."
    fi

else
    echo "Installation de la règle udev backlight..."

    # Création de la règle udev
    echo 'SUBSYSTEM=="backlight", ACTION=="add", RUN+="/bin/chmod 666 /sys/class/backlight/%k/brightness"' | sudo tee "$RULE_FILE" > /dev/null

    # Recharge des règles udev
    sudo udevadm control --reload-rules
    sudo udevadm trigger

    echo "✔ Règle installée avec succès."
fi
